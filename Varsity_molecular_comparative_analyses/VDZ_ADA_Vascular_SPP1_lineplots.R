...
npx_mat <- read.table(file.path(dir.dat, "NPX_expression_all_filtered.txt"), sep = "\t", header = TRUE, row.names = 1)
...
keep.vars <- c("SampleID", "Subject.ID", "PlateID", "Group", "Treatment", "Week", "Response_W14", "Remission_W52")
...
responses <- c("R_W52")
covariates_cols <- c("PlateID", "Subject.ID")
...
df_titles <- data.frame(
  ...
  "title" = c("Clinical Response W14", "Clinical Remission W52"),
  "cap" = c("Clinical", "Clinical"),
  "week" = c("W14", "W52")
)
